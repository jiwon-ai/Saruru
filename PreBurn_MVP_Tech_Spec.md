...
1. **Local-first.** Entries live on-device. No server stores emotional text. Delete-after-burn is ON by default.
...
| Animation | **react-native-reanimated** (+ Lottie later) | Burn ritual |
...
export type ReframeResult = {
  ...
  burn_sentence: string;        // short line shown on the burning object
  safety: { flag: boolean; reason: string | null };
};

export type Settings = {
  deleteAfterBurn: boolean;     // default true
};
```

**Delete-after-burn (default ON):** when an incident is burned, the `text` and `reframe` are erased; only `{ id, burnedAt }` is kept so the streak count works. If the user turns it OFF, the entry is kept in a private local archive (Plus feature later).
...
```
Home ──"New burn"──▶ Capture ──"Continue"──▶ Reframe ──"Burn it"──▶ Burn ──▶ Released ──▶ Home
                                              │
                                              └─ safety.flag = true ─▶ Crisis
```

- **Home (`index`)** — app name, streak count (`N days, residue released`), one calm line, `+ New burn` button. Privacy lock badge visible.
...
- **Reframe (`reframe`)** — on mount, call `getReframe(incident)`. Show a loading state, then render `not_your_responsibility`, `why_it_stung`, `closure_reframe`. `boundary_line` shown only if present, as a small secondary card. `Burn it` → Burn. **If `safety.flag` → replace this screen with Crisis.**
- **Burn (`burn`)** — render the incident as a paper/receipt object showing `burn_sentence`; Reanimated sequence folds → darkens → burns → fades to ash. Then Released.
- **Released (`released`)** — "Released. You do not have to carry this." Run delete-after-burn if enabled. Increment streak. Back to Home.
...
### 6.4 Cost control
- One Claude call per burn. Haiku keeps it cheap; that single call also does safety detection (no separate model needed for v1).
- Cap free-tier burns; the AI call is the only real variable cost. (Monetization deferred — see plan.)

---

## 7. Privacy & safety implementation checklist

- [ ] No analytics SDK that ships incident text off-device. If you add analytics, send only anonymous events (e.g. `burn_completed`), never the text.
- [ ] Proxy logs **nothing** containing the incident text. Disable request-body logging.
- [ ] Delete-after-burn default ON; offer explicit "keep in private archive" opt-in.
...
7. Add the burn animation (start simple: fade + scale; polish later).
8. Implement delete-after-burn + streak.
...
3. **Streak storage:** keep only `{date}` of each burn; nothing else if delete-after-burn is on.
4. **Structured output hardening:** plain-JSON prompt now; migrate to Claude tool-use for guaranteed schema later.
5. **Offline:** the burn ritual itself should work offline; only the reframe needs network. Consider a non-AI "quick burn" that skips Claude (also your future free tier).
